...
@implementation NewAuditionsViewController
...
{
    if(clearData)
        [LoginInfo sharedInstance].newAuditionData = [[NSMutableArray alloc] init];
    [[API sharedInstance] getCommand:nil  APIPath:[NSString stringWithFormat:@"/Api/GetVideos/?fromIndex=%lu%@",(unsigned long)[[LoginInfo sharedInstance].newAuditionData count],
        ...
        if ([json valueForKey:@"error" ]) {
			...
			for(id video in [json valueForKey:@"videos"]   )
            {
                ...
                [[LoginInfo sharedInstance].newAuditionData addObject:auditionRecord];
            }
...
{
    if([[LoginInfo sharedInstance].newAuditionData count]-1==indexPath.row)
    {
        [self loadData:false];
...
{
   return [[LoginInfo sharedInstance].newAuditionData count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Set up the cell...
    Audition *appRecord = [[LoginInfo sharedInstance].newAuditionData objectAtIndex:indexPath.row ] ;
...
-(void) play:(id) sender{
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@/Videos/%@.mp4", kWEBHost, [[[LoginInfo sharedInstance].newAuditionData objectAtIndex:play.tag] valueForKey:@"ID"]]];
    ...
    NSMutableDictionary* params =[NSMutableDictionary dictionaryWithObjectsAndKeys:[[[LoginInfo sharedInstance].newAuditionData objectAtIndex:play.tag] valueForKey:@"ID"], @"videoId",
...
{
    if ([[LoginInfo sharedInstance].newAuditionData count] > 0)
    {
